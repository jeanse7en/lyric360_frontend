...
export default function LyricPanel({ lyrics, selectedUrl, onSelect, onFullscreen, onPresent, hasSong }: Props) {
  return (
              ...
              <div key={i} className="flex gap-1">
                <button
                  ...
                  className={`px-3 py-1 rounded-l text-xs font-medium transition-colors ${
                    isSelected ? 'bg-pink-500 text-white' : 'bg-gray-700 hover:bg-gray-600 text-white'
                  }`}
                >
                  Lời {i + 1}
                </button>
                <button
                  onClick={() => onPresent(url)}
                  className="px-2 py-1 rounded-r text-xs font-medium bg-green-700 hover:bg-green-600 text-white transition-colors"
                  title="Hiển thị cho người hát"
                >
                  📺
                </button>
              </div>
